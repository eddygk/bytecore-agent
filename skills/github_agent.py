...
class GitHubAgentSkill(BaseSkill):
    ...
    async def execute(self, action: str, repo: Optional[str] = None, **kwargs) -> Any:
        """
    ...
    async def _list_issues(self, state: str = "open", **kwargs) -> Dict[str, Any]:
        ...
        return {"issues": issue_list, "total_count": issues.totalCount}

    async def _close_issues(
        self, labels: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        for issue in issues:
            ...
            if should_close:
                try:
                    ...
                    closed_count += 1
                    closed_issues.append({"number": issue.number, "title": issue.title})
                except Exception as e:
                    self.logger.error(f"Failed to close issue #{issue.number}: {e}")

        return {"closed_count": closed_count, "closed_issues": closed_issues}

    async def _create_issue(
        self, title: str, body: str, labels: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        """Create a new issue."""
        if not self.current_repo:
            return {"error": "No repository specified"}
    ...
    async def _list_pull_requests(
        ...
        return {"pull_requests": pr_list, "total_count": prs.totalCount}
